...
class MantarixServer implements MantarixControlBackend {
  ...
  MantarixServer(this._store, this.controlInvokeMethods,
      {this.reconnectIntervalMs, this.reconnectTimeoutMs, this.errorsHandler});

  Future connect({required String address}) async {
    _address = address;

    debugPrint("Connecting to Mantarix server $address...");
    try {
      _clientProtocol = MantarixServerProtocol(
          address: _address,
      ...
      debugPrint("Error connecting to Mantarix server: $e");
      _onDisconnect();
    }
  }
  ...
  _onDisconnect() {
    ...
    if (reconnectTimeoutMs == null ||
      ...
      errorsHandler?.onError(
          "Error connecting to a Mantarix service in a timely manner.");
    }
  }
  ...
  void disconnect() {
    debugPrint("Disconnecting from Mantarix server.");
    _disposed = true;
