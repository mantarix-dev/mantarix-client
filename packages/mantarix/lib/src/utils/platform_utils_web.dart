...
String getWebsocketEndpointPath(String uriPath) {
  var meta = html.document.head
      ?.querySelector("meta[name='mantarix-websocket-endpoint-path']");
  return trim(meta?.attributes["content"] ?? "ws", "/");
}

String getMantarixRouteUrlStrategy() {
  var meta = html.document.head
      ?.querySelector("meta[name='mantarix-route-url-strategy']");
  return meta != null ? meta.attributes["content"]! : "";
}

bool isMantarixWebPyodideMode() {
  var meta =
      html.document.head?.querySelector("meta[name='mantarix-web-pyodide']");
  return meta != null
