...
import 'package:mantarix/src/utils/badge.dart';
...
import 'mantarix_app_control.dart';
    ...
    builder: (context, controlView) {
      ...
      if (key != "") {
        if (key.startsWith("test:")) {
          ...
          MantarixAppServices.of(context).globalKeys[key] = globalKey;
        }
      }
      ...
      for (var createControlFactory
          in MantarixAppServices.of(context).createControlFactories) {
        widget = createControlFactory(CreateControlArgs(
            ...
            parentDisabled,
            parentAdaptive,
            MantarixAppServices.of(context).server));
        if (widget != null) {
          break;
        }
      }

      // try creating Mantarix built-in widget
      widget ??= createWidget(controlKey, controlView, parent, parentDisabled,
          parentAdaptive, nextChild, MantarixAppServices.of(context).server);
...
Widget createWidget(
    ...
    bool? parentAdaptive,
    Widget? nextChild,
    MantarixControlBackend backend) {
  ...
  if (animation != null) {
    return AnimatedOpacity(
      ...
      onEnd: control.attrBool("onAnimationEnd", false)!
          ? () {
              MantarixAppServices.of(context)
                  .server
...
Widget _rotatedControl(
  ...
  if (animation != null) {
    return AnimatedRotation(
        ...
        onEnd: control.attrBool("onAnimationEnd", false)!
            ? () {
                MantarixAppServices.of(context).server.triggerControlEvent(
