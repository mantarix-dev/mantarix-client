...
class _FilePickerControlState extends State<FilePickerControl>
  ...
  Widget build(BuildContext context) {
    ...
    return withPageArgs((context, pageArgs) {
      ...
      if (_upload != upload && upload != null && _files != null) {
        _upload = upload;
        uploadFiles(
            upload, MantarixAppServices.of(context).server, pageArgs.pageUri!);
      }
  ...
  Future uploadFiles(
      String filesJson, MantarixServer server, Uri pageUri) async {
  ...
  Future uploadFile(PlatformFile file, MantarixServer server, String uploadUrl,
      String method) async {
  ...
  void sendProgress(
      MantarixServer server, String name, double? progress, String? error) {
    widget.backend.triggerControlEvent(
        widget.control.id,
        "upload",
